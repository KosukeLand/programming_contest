...
func main() {
	...
	NG1, _ = strconv.Atoi(read())
	NG2, _ = strconv.Atoi(read())
	NG3, _ = strconv.Atoi(read())
...
func dfs(n, cnt int) int {
	...
	memo[n] = max(dfs(n-3, cnt), dfs(n-2, cnt), dfs(n-1, cnt))
	return memo[n]
}
