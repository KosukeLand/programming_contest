...
func main() {
	...
	xa, _ := strconv.Atoi(read())
	ya, _ := strconv.Atoi(read())
	xb, _ := strconv.Atoi(read())
	yb, _ := strconv.Atoi(read())
	xc, _ := strconv.Atoi(read())
	yc, _ := strconv.Atoi(read())

	A := math.Sqrt(float64(pow(xa-xb, 2) + pow(ya-yb, 2))) // a-b距離
	B := math.Sqrt(float64(pow(xb-xc, 2) + pow(yb-yc, 2))) // b-c距離
	C := math.Sqrt(float64(pow(xc-xa, 2) + pow(yc-ya, 2))) // c-a距離
