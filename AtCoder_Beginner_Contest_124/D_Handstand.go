...
func main() {
	...
	now := 1
	for i := 0; i < N; i++ {
		if string(S[i]) != strconv.Itoa(now) {
			now = 1 - now
			num = append(num, cnt)
			cnt = 1
		} else {
	...
	table := make([]int, len(num)+1)
	for i := 1; i < len(table); i++ {
		table[i] += table[i-1] + num[i-1]
	}

	if 2*K < len(table) {
		for left := 0; left+2*K < len(table); left = left + 2 {
			right := left + 2*K + 1
			ans = max(ans, table[right]-table[left])
		}
		fmt.Println(ans)
	} else {
